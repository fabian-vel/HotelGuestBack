...
public class GlobalErrorWebExceptionHandler implements ErrorWebExceptionHandler {
    ...
    private ErrorDescriptor resolve(Throwable ex) {
        return switch (ex) {                                 // ← switch pattern (Java 21)
            ...
            case BusinessException e      -> new ErrorDescriptor(HttpStatus.UNPROCESSABLE_ENTITY,  "BUSINESS_ERROR", e.getUserMessage());
            case InternalServerErrorException e -> new ErrorDescriptor(HttpStatus.INTERNAL_SERVER_ERROR, "INTERNAL_ERROR", e.getUserMessage());
            case SqlLoadException e        -> new ErrorDescriptor(HttpStatus.INTERNAL_SERVER_ERROR, "CONFIG_ERROR", "Error de configuración interna");
            case AppException e            -> new ErrorDescriptor(e.getHttpStatus(),               "APP_ERROR",    e.getUserMessage());
            default                        -> new ErrorDescriptor(HttpStatus.INTERNAL_SERVER_ERROR, "INTERNAL_ERROR", "Error interno del servidor");
        };
    }
